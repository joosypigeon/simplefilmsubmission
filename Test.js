...
function myTests() {
  ...
  // Create a temporary spreadsheet and put it in the folder Test which resides at the root of the drive
  ...
  var ssFile = DocsList.getFileById(ss.getId()),
    ssParentFolder = ssFile.getParents()[0],
    testFolder = DocsList.createFolder('Test');

  ssFile.addToFolder(testFolder);
  ssFile.removeFromFolder(ssParentFolder);

  ScriptProperties.deleteProperty(normalizeHeader(LOG_FILE));
...

{
  ...
  test('setup', function () {
    ...
    for(i = 0; i < spreadsheetValues.length; i++) {
      ...
      ScriptProperties.deleteProperty(normalizeHeader(TEMPLATES_TESTING));
      ...
      templatesTesting = ScriptProperties.getProperty(normalizeHeader(TEMPLATES_TESTING));
      if((+i) < overQuota) {
        if(templatesTesting) {
...
log('+++++++++++++++++++');
log('+++++++++++++++++++');
log('Test Ad Hoc Email sent before close of submission');
...
log('+++++++++++++++++++');
log('+++++++++++++++++++');
log('Test Ad Hoc Email sent after close of submission');
...
for(var ii = 0; ii<templates.length; ii++){
  ...
  //DocsList.getFileById(ss.getId()).setTrashed(true);
}